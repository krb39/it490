...
will build a web application that uses our messaging framework for any
communication that isn't with the user:
...
the *Front End* developer is comfortable working with and what features they
would like. The most common web frameworks for various languages are shown
below. You are encouraged to research them and determine what would best fit
your needs.
...
* https://koajs.com/[Koa]
https://www.php.net/[PHP]:: Originally a templating language, PHP is still
popular and can be used as a framework.
...
extensive documentation on using the most popular messaging libraries for
various languages:
...
JavaScript:: Frameworks make use of Node and typically use a
...
interface _and_ communicates with a messaging container. We will be following
along with the https://www.rabbitmq.com/tutorials/tutorial-six-php.html[
RabbitMQ RPC tutorial for PHP] and using the https://hub.docker.com/_/php[
official PHP Docker Hub image].
...
.{sourcedir}/web/app/composer.json
[source, json]
----
include::{sourceroot}/{sourcedir}/web/app/composer.json[]
----
...
[source, console]
----
PS front-end-demo\web\app> docker run --rm -it -v "$(pwd):/app" composer install <1>
...
bind link the app directory `front-end-demo/web/app` (which we are in) with the
...
.{sourcedir}/web/Dockerfile
[source, docker]
----
include::{sourceroot}/{sourcedir}/web/Dockerfile[]
----
...
the `docker-compose.yml` still creates a bind mount. This allows us to actively
develop the PHP code while the container is running. When we are done
developing we can comment out the bind mount.
...
`echo` method that sends JSON data.
...
TIP: Give some thought to your exchange formats. Try to use something that both
*Front End* and *Back End* can support. JSON is currently the most common
choice.

.{sourcedir}/web/app/rpc_client.php
[source, php]
----
include::{sourceroot}/{sourcedir}/web/app/rpc_client.php[]
----

To test out our `RpcClient` class, let's create a `echo.php` endpoint that takes
a POST parameter of msg and calls the echo procedure with msg's contents:

.{sourcedir}/web/app/echo.php
[source, php]
----
include::{sourceroot}/{sourcedir}/web/app/echo.php[]
----

We'll also need a basic form to test it out:

.{sourcedir}/web/app/echo.html
[source, html]
----
include::{sourceroot}/{sourcedir}/web/app/echo.html[]
----
...
a link that they can just click on and take an action, ie:
http://localhost:8080/echo.php?msg=do+bad+things.
...
http://localhost:8080/echo.html to test the functionality:

[source, console]
----
PS C:\Users\rxt1077\it490\front-end-demo> docker-compose up
...
web_1        | 172.19.0.1 - - [14/May/2020:01:36:09 +0000] "GET /echo.html HTTP/1.1" 200 484 "-" "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:76.0) Gecko/20100101 Firefox/76.0" <1>
...
web_1        | 172.19.0.1 - - [14/May/2020:01:36:13 +0000] "POST /echo.php HTTP/1.1" 200 718 "http://localhost:8080/echo.html" "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:76.0) Gecko/20100101 Firefox/76.0" <4>
----
<1> The web service will report the pages being served as you are testing it.
...
If you test it, I used the text "This is a test", you should see the following
output after 10 seconds:

[source, plaintext]
----
Establishing connection to messaging service... [SUCCESS] <1>
Sending "This is a test" to the echo procedure and waiting for response...
...
[qanda]
Why are we using a messaging layer between for communications to / from *Front End*?::
    {empty}
