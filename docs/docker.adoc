...
Every VM having to run its own OS creates and emulate a unique machine creates a
lot of overhead. Containers allow for similar isolation as VMs, but at a reduced
resource cost since. As opposed to VMs which emulate a physical machine,
containers use
...
* Windows 10 Home _does not_ have Hyper-V available, but you can easily
  ...
  University) and then instal it. This should be your first step if
...
* https://docs.docker.com/toolbox/[Docker Toolbox] is an older version of
  ...
  and Docker Desktop is preferred.
Mac::
* https://hub.docker.com/?overlay=onboarding>[Docker Desktop] should run fine
