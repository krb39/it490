...
Traditionally, virtualization has made use of the virtual machine (VM) to
...
====
Prithi is in charge of maintaining a payroll system written using Perl, a web
...
module from their environment due to security concerns.

To solve this problem, Prithi asks the team that maintains the servers if they
...
limited to just the payroll system. The web servers can still be upgraded _and_
Prithi can maintain the payroll system thanks to virtualization with VMs.
====
...
Every VM having to run its own OS creates a lot of overhead and you can't fit
...
https://linuxcontainers.org/[LXC], https://containerd.io/[containerd], and
https://www.docker.com/[Docker]. We will be using Docker due to its file format
...
on any other machine that runs Docker.
...
* https://hub.docker.com/?overlay=onboarding[Docker Desktop] should run fine
  on any Windows system that has Hyper-V (a virtualization feature) available.
* Windows 10 Home _does not_ have Hyper-V available, but you can easily
  https://answers.microsoft.com/en-us/windows/forum/windows_10-update/how-to-convert-windows-10-home-to-education/5a6e42b0-51fc-4e53-9497-454c3adc671c?auth=1[
  Upgrade to Windows 10 Education] (typically this is provided by your
  University) and then try installing it. This should be your first step if
  Docker Desktop does not work out-of-the-box.
* https://docs.docker.com/toolbox/[Docker Toolbox] is an older version of
  Docker, running under a VM in VirtualBox. It can be difficult to work with
  and I highly recommend Docker Desktop if possible.
Mac::
* https://hub.docker.com/?overlay=onboarding>[Docker Desktop] should run fine
...
* If for some reason your computer is not supported you can try the macOS version
  of https://docs.docker.com/toolbox/[Docker Toolbox].
...
tagged using the format `name:version`. One of the reasons Docker has become so
...
attempt to `pull` that image from the https://hub.docker.com/[Docker Hub]
container registry.
...
[plantuml, images-containers-registry, svg]
....
@startuml
...
Docker commands are run via the `docker` command line interface in a terminal.
Here is a _brief_ listing of some of the most useful commands:

images::
Lists images that are locally available.
ps::
Lists the currently _running_ containers.
pull::
...
running Docker for a while, this can free up GB of space.
...
you installed Docker Desktop, or Terminal if you are in MacOS.
...
----
<1> `docker ps` shows all _running_ containers, to see _all_ containers we
could have used `docker ps -a`.
...
a web browser and go to http://localhost:8080 and see a page stating, "It
works!"
...
<3> It is still there, just not running as evidenced by the `docker ps -a`
command.
<4> `docker rm` will remove it for good.

In most situations, `docker rm` isn't really necessary. As stopped containers
do not actively consume resources. The browsing to http://localhost:8080 again
to confirm that the container is actually shut down.
...
do this in the `{sourcedir}` directory:
...
this warning.
<4> Finally we look at the images currently available and see that the one we
built is available.
...
Having to specify command line arguments for the docker command can get tedious,
...
Alright, now let's bring up the web service:
...
<2> Docker Compose also create a network for your services to run in. This
helps with service communication later on.
...
We can test that things are working by going to http://localhost:8080. Then
