...
server, but uses *Messaging* for any other communication:
...
interface _and_ communicates with *Messaging*. We will be basing our code of of
...
If you test it, I used the ECHO action and the text "This is a test", you should
see the following output after 10 seconds:
...
[qanda]
Why are we using a messaging layer for communications to / from *Front End*?::
    {empty}
